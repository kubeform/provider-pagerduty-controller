...
// +kubebuilder:rbac:groups=tag.pagerduty.kubeform.com,resources=assignments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=tag.pagerduty.kubeform.com,resources=assignments/status,verbs=get;update;patch
...
func (r *AssignmentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Assignment")
...
func (r *AssignmentReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &tagv1alpha1.Assignment{}); err != nil {
			klog.Error(err, "unable to set up auditor", tagv1alpha1.Assignment{}.APIVersion, tagv1alpha1.Assignment{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&tagv1alpha1.Assignment{}).
