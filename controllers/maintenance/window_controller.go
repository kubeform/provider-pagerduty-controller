...
// WindowReconciler reconciles a Window object
...
// +kubebuilder:rbac:groups=maintenance.pagerduty.kubeform.com,resources=windows,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=maintenance.pagerduty.kubeform.com,resources=windows/status,verbs=get;update;patch

func (r *WindowReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("window", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Window")
...
func (r *WindowReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &maintenancev1alpha1.Window{}); err != nil {
			klog.Error(err, "unable to set up auditor", maintenancev1alpha1.Window{}.APIVersion, maintenancev1alpha1.Window{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&maintenancev1alpha1.Window{}).
