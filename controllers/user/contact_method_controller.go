...
// ContactMethodReconciler reconciles a ContactMethod object
type ContactMethodReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=user.pagerduty.kubeform.com,resources=contactmethods,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=user.pagerduty.kubeform.com,resources=contactmethods/status,verbs=get;update;patch

func (r *ContactMethodReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("contactmethod", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ContactMethod")
...
func (r *ContactMethodReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &userv1alpha1.ContactMethod{}); err != nil {
			klog.Error(err, "unable to set up auditor", userv1alpha1.ContactMethod{}.APIVersion, userv1alpha1.ContactMethod{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&userv1alpha1.ContactMethod{}).
