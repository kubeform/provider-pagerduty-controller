...
// +kubebuilder:rbac:groups=ruleset.pagerduty.kubeform.com,resources=rulesets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=ruleset.pagerduty.kubeform.com,resources=rulesets/status,verbs=get;update;patch

func (r *RulesetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("ruleset", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Ruleset")
...
func (r *RulesetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &rulesetv1alpha1.Ruleset{}); err != nil {
			klog.Error(err, "unable to set up auditor", rulesetv1alpha1.Ruleset{}.APIVersion, rulesetv1alpha1.Ruleset{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&rulesetv1alpha1.Ruleset{}).
