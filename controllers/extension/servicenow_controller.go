...
// ServicenowReconciler reconciles a Servicenow object
type ServicenowReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=extension.pagerduty.kubeform.com,resources=servicenows,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=extension.pagerduty.kubeform.com,resources=servicenows/status,verbs=get;update;patch

func (r *ServicenowReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicenow", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Servicenow")
...
func (r *ServicenowReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &extensionv1alpha1.Servicenow{}); err != nil {
			klog.Error(err, "unable to set up auditor", extensionv1alpha1.Servicenow{}.APIVersion, extensionv1alpha1.Servicenow{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&extensionv1alpha1.Servicenow{}).
...
func (r *ServicenowReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Servicenow")
			return nil
		}
		...
		resourceList := &extensionv1alpha1.ServicenowList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Servicenow")
			return nil
		}
