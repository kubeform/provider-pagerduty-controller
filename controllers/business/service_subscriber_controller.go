...
// ServiceSubscriberReconciler reconciles a ServiceSubscriber object
type ServiceSubscriberReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=business.pagerduty.kubeform.com,resources=servicesubscribers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=business.pagerduty.kubeform.com,resources=servicesubscribers/status,verbs=get;update;patch

func (r *ServiceSubscriberReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("servicesubscriber", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ServiceSubscriber")
...
func (r *ServiceSubscriberReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &businessv1alpha1.ServiceSubscriber{}); err != nil {
			klog.Error(err, "unable to set up auditor", businessv1alpha1.ServiceSubscriber{}.APIVersion, businessv1alpha1.ServiceSubscriber{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&businessv1alpha1.ServiceSubscriber{}).
