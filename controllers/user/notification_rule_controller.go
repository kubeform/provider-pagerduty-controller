...
// +kubebuilder:rbac:groups=user.pagerduty.kubeform.com,resources=notificationrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=user.pagerduty.kubeform.com,resources=notificationrules/status,verbs=get;update;patch

func (r *NotificationRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("notificationrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch NotificationRule")
...
func (r *NotificationRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &userv1alpha1.NotificationRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", userv1alpha1.NotificationRule{}.APIVersion, userv1alpha1.NotificationRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&userv1alpha1.NotificationRule{}).
