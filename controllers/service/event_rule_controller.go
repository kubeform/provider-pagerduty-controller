...
// EventRuleReconciler reconciles a EventRule object
...
// +kubebuilder:rbac:groups=service.pagerduty.kubeform.com,resources=eventrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=service.pagerduty.kubeform.com,resources=eventrules/status,verbs=get;update;patch

func (r *EventRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("eventrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch EventRule")
...
func (r *EventRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicev1alpha1.EventRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicev1alpha1.EventRule{}.APIVersion, servicev1alpha1.EventRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicev1alpha1.EventRule{}).
