...
// +kubebuilder:rbac:groups=tag.pagerduty.kubeform.com,resources=tags,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=tag.pagerduty.kubeform.com,resources=tags/status,verbs=get;update;patch

func (r *TagReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tag", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Tag")
...
func (r *TagReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &tagv1alpha1.Tag{}); err != nil {
			klog.Error(err, "unable to set up auditor", tagv1alpha1.Tag{}.APIVersion, tagv1alpha1.Tag{}.Kind)
			return err
		}
	}
